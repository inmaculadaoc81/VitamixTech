...
export default async function handler(req,res){
  ...
  try{
    ...
    const subject=`Nueva consulta VitamixTech - vitamixtech.com.es | ${modelo||'Batidora Vitamix'}`;
    const html=`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#241B2F"><h2 style="color:#7A2E52">Nueva consulta desde VitamixTech</h2><p><strong>Nombre:</strong> ${safe.nombre}</p><p><strong>Teléfono:</strong> ${safe.telefono}</p><p><strong>Email:</strong> ${safe.email}</p><p><strong>Modelo Vitamix:</strong> ${safe.modelo}</p><p><strong>Descripción de la avería:</strong><br>${safe.mensaje}</p><hr><p style="font-size:12px;color:#6b5f66">Enviado desde el formulario de vitamixtech.com.es</p></div>`;
    const mime=[`From: VitamixTech <${from}>`,`To: ${to}`,`Reply-To: ${email}`,`Subject: ${encodeHeader(subject)}`,'MIME-Version: 1.0','Content-Type: text/html; charset="UTF-8"','Content-Transfer-Encoding: 8bit','',html].join('\r\n');
